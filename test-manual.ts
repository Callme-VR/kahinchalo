...
async function testAPIs() {
  ...
  try {
    ...
    const reviewsResponse = await fetch(`${API_BASE}/vendors/${testVendorId}/reviews`);
    ...
    console.log('   (Categories should be created via database or admin panel)');
    ...
    // Test Get Trips
    console.log('\n🏔️ Testing Get Trips...');
    const tripsResponse = await fetch(`${API_BASE}/trips?ageGroup=18-35&budget=500`);
    
    if (tripsResponse.status === 200) {
      const tripsData = await tripsResponse.json();
      console.log('✅ Get trips successful');
      console.log(`   Trips count: ${tripsData.trips.length}`);
      console.log(`   Pagination: page ${tripsData.pagination.page} of ${tripsData.pagination.totalPages}`);
    ...
    if (testTripId) {
      ...
      const tripDetailsResponse = await fetch(`${API_BASE}/trips/${testTripId}`);
    ...
    if (testCategoryId) {
      ...
      const categoryTripsResponse = await fetch(`${API_BASE}/categories/${testCategoryId}/trips`, {
        method: 'POST'
      });
      ...
      if (categoryTripsResponse.status === 200) {
        ...
        console.log('❌ Get trips by category failed:', categoryTripsResponse.status);
      }
    }
    ...
    console.log('   ✅ POST /vendors/documents - Upload verification docs (requires auth)');
